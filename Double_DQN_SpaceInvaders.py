...
class DQN_Agent():
            ...
            while True:#each step
                ...
                if len(frames_4) > 4:
                    ...
                    if len(state_buff)>1:# we save new transition into the memory
                        ...
                        #self.replay_memory.append([old_state_stack,cur_action,sum(reward_list),state_stack,is_terminal]) #state, action, reward, next state, terminal flag tuples.
    ...
    def burn_in_memory(self):
        ...
        #print(len(self.replay_memory.memory),233323232)
...
def rgb2gray(rgb):
    ...
    #pixel =np.dot(rgb[...,:3], [0.299, 0.587, 0.114])
